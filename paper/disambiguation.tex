...
\begin{document}
	...
	This paper investigates a different hypothesis: global entity resolution may not require global mention-level reasoning. Instead, most entity structure may be recoverable through local resolution within bounded contexts, after which global consistency can be reconstructed through interactions between locally induced entity clusters. If true, the problem can be reformulated from global pair-wise mention comparison to local nominal resolution and matching predicted clusters. The proposed system consists of three components:
	\begin{enumerate}
		...
		\item \textbf{Cluster-level global matching:} Local clusters are treated as nodes in a quotient graph. Cross-window entity consistency is recovered by learning a matching function over cluster representations rather than directly comparing individual mentions. We empirically demonstrate that global reasoning over quotient-graph cluster representations outperforms direct global mention-level reasoning under identical frozen encoders and training conditions.
	\end{enumerate}
	...
	\begin{itemize}
		...
		\item We provide empirical evidence that local clustering captures sufficient entity structure to enable recovery of full-document entities through cluster-level reconciliation.
	\end{itemize}
	...
	\begin{hypothesis}[Transitivity chain and locality]
		Any two mentions of an entity can be connected through a chain of locally supported references. That is, $\forall e\in \mathcal{E},\forall u,v\in\mathcal{E},\exists m_{1},\ldots,m_{r}\in\mathcal{E}$ such that  $q(m_1) = q(u), q(m_r) = q(v)$ and  $|i_{j+1}-i_{j}|\leq K$.
	\end{hypothesis}
	...
	\begin{table}[ht]
		...
		\begin{tabular}{lcccc}
			...
			Stage A only / Model A (full-doc gold) & 0.7428 & 0.8442 & 0.6896 & 0.6947 \\
			...
			\bottomrule
		\end{tabular}
		\caption{Full-document CoNLL-2012 test results on \textbf{gold mention boundaries} \citep{pradhan2012conll}. All baseline configurations are mapped strictly to their gold-mention settings to isolate coreference logic from mention detection errors.}
		\label{tab:stageb}
	\end{table}
	...
	\end{table}
	Evaluated directly against full-document gold, Stage A alone (Model A: every window-local cluster left unmerged) scores 0.7428, suffering a loss due to entity fragmentation at window boundaries. Stage B exists specifically to recover these missing global links. It is therefore not a post-processing heuristic but the mechanism that reconstructs the full-document entity partition from locally resolved components. Stage B recovers $+0.11$ CoNLL points over Stage A alone on full-document gold ($0.7428 \to 0.8533$). Two facts follow. First, local-only resolution already attains 87\% of the final F$_1$, confirming that the majority of coreference is decidable within a single bounded window with no global context. Second, the cross-window step is nonetheless substantial: the $+0.11$ gain is concentrated in the cluster-level metrics (B$^3$ $0.690 \to 0.832$, CEAFe $0.695 \to 0.809$) while MUC barely moves ($0.844 \to 0.919$) --- the precise signature of fragmented entities being rejoined rather than new links being invented. PRON-only (0.664) remains the hardest bucket: pronouns carry an uninformative signature inside the semantic channel, forcing Stage B to rely entirely on context profiles which naturally decay across window gaps. Treating each accepted Stage B merge as a quotient-graph edge, the cross-window matcher operates at 0.781 edge precision and 0.686 edge recall (over cross-window gold cluster pairs) against gold.
