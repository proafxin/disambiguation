...
def _snap_to_constituent(spans: list, sentences: list, sdoc) -> list:
    # snap each word span (si, a, b) to its head's dependency subtree (the full NP, modifiers included),
    # clamped to the sentence. Short spans (pronouns/names) are their own subtree -> unchanged.
    offsets, off = [], 0
    for s in sentences:
        offsets.append(off)
        off += len(s)
    out = []
    for si, a, b in spans:
        gs, ge = offsets[si] + a, offsets[si] + b
        toks = [sdoc[i] for i in range(gs, ge) if i < len(sdoc)]
        if not toks:
            out.append((si, a, b))
            continue
        roots = [t for t in toks if not (gs <= t.head.i < ge) or t.head.i == t.i]
        root = roots[0] if roots else toks[len(toks) // 2]
        sent_s, sent_e = offsets[si], offsets[si] + len(sentences[si])
        sub = [t.i for t in root.subtree if sent_s <= t.i < sent_e]
        if not sub:
            out.append((si, a, b))
            continue
        out.append((si, min(sub) - sent_s, max(sub) + 1 - sent_s))
    return out


def test_constituent_snap(thr: float = 0.2, device: str = "cuda" if torch.cuda.is_available() else "cpu") -> dict:
    # Does snapping predicted spans to syntactic constituents raise EXACT recall vs gold? (the -6.6 fix)
    import spacy
    import spacy.tokens
    from datasets import load_from_disk

    from disambiguation.paths import DATA_DIR, SPACY_TRF_DIR

    docs = build_predicted_docs(str(MODELS_DIR / "bio_tagger_k510_L3_sent.pt"), thr=thr, device=device)
    by_words = {tuple(w for s in d["sentences"] for w in s): d for d in docs if d["split"] == "test"}
    vocab = spacy.blank("en").vocab
    ds = load_from_disk(str(DATA_DIR / "conll2012"))["test"]
    db = spacy.tokens.DocBin().from_disk(SPACY_TRF_DIR / "conll2012_test.spacy")
    ex_raw = ex_snap = ex_nc = ov = gold_tot = 0
    for sample, sdoc in zip(ds, db.get_docs(vocab), strict=False):
        d = by_words.get(tuple(w for s in sample["sentences"] for w in s))
        if d is None:
            continue
        gold = {tuple(g) for g in d["gold_spans"]}
        raw = [tuple(s) for s in d["spans"]]
        snap = _snap_to_constituent(raw, d["sentences"], sdoc)
        # noun-chunk snap: extend each span to the base noun chunk it overlaps most (milder than subtree)
        off2, off = [], 0
        for s in d["sentences"]:
            off2.append(off)
            off += len(s)
        chunks = list(sdoc.noun_chunks)
        ncsnap = []
        for si, a, b in raw:
            gs, ge = off2[si] + a, off2[si] + b
            best, bo = None, 0
            for c in chunks:
                o = max(0, min(ge, c.end) - max(gs, c.start))
                if o > bo:
                    bo, best = o, c
            ncsnap.append((si, best.start - off2[si], best.end - off2[si]) if best else (si, a, b))
        gold_tot += len(gold)
        ex_raw += len(gold & set(raw))
        ex_snap += len(gold & set(snap))
        ex_nc += len(gold & set(ncsnap))
        for r, sp in zip(raw, ncsnap, strict=True):
            if r not in gold:
                ov += int(sp in gold)
    print(f"\n=== Snap test (TEST) — exact recall vs gold ===")
    print(f"gold mentions {gold_tot}")
    print(f"RAW           {100 * ex_raw / max(gold_tot, 1):.2f}% ({ex_raw})")
    print(f"subtree-snap  {100 * ex_snap / max(gold_tot, 1):.2f}% ({ex_snap})  ({ex_snap - ex_raw:+d})")
    print(f"nounchunk-snap{100 * ex_nc / max(gold_tot, 1):.2f}% ({ex_nc})  ({ex_nc - ex_raw:+d}) | fixed {ov} non-exact")
    return {"raw": ex_raw, "subtree": ex_snap, "nounchunk": ex_nc, "gold": gold_tot}
