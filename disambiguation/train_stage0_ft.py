...
def load_items(split: str, bge: SentenceTransformer) -> list:
    ...
    for s in ds:
        ...
        for si, words in enumerate(s["sentences"]):
            ...
            items.append({"words": words, "spans": sp.get(si, []), "rec": rec, "sem_rec": sem_rec[si]})
    return items


class FTDetector(nn.Module):
    def __init__(self, proj: int = 512, dropout: float = 0.2, width_dim: int = 64, rec_dim: int = 32, sem_dim: int = 32):
        super().__init__()
        ...
        self.rec_emb = nn.Embedding(5, rec_dim)           # binned doc-frequency (lexical recurrence)
        self.sem_emb = nn.Embedding(8, sem_dim)           # binned max BGE cosine sim (semantic recurrence)
        self.register_buffer("rec_bounds", torch.tensor([1.5, 2.5, 4.5, 9.5]))
        self.register_buffer("sem_bounds", torch.tensor([0.5, 0.6, 0.7, 0.75, 0.8, 0.85, 0.9]))
        self.ffnn = nn.Sequential(
            nn.Linear(3 * d + width_dim + 2 * rec_dim + 2 * sem_dim, proj), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(proj, 1),
        )

    def forward(self, input_ids, attn_mask, word_first, rec, sem_rec):
        ...
        hw = torch.gather(h, 1, idx)                       # (B, W, d) per-word (first subtoken)
        B, W, d = hw.shape
        L = MAX_MENTION_LEN
        # attention-pooled span content via prefix sums, band-limited to spans of length <= L:
        # content(i, i+k) = sum_{t in [i, i+k]} softmax(a)_t * h_t
        a = self.attn(hw).squeeze(-1)                      # (B, W)
        e = torch.exp(a - a.amax(dim=1, keepdim=True))     # (B, W) softmax numerators
        z = torch.zeros(B, 1, device=hw.device, dtype=hw.dtype)
        zh = torch.zeros(B, 1, d, device=hw.device, dtype=hw.dtype)
        pe = torch.cat([z, e.cumsum(1)], dim=1)            # (B, W+1)
        peh = torch.cat([zh, (e.unsqueeze(-1) * hw).cumsum(1)], dim=1)  # (B, W+1, d)
        i_idx = torch.arange(W, device=hw.device).view(W, 1)   # (W,1) start
        k_idx = torch.arange(L, device=hw.device).view(1, L)   # (1,L) offset (end j = i+k)
        end = (i_idx + k_idx + 1).clamp(max=W)             # (W,L) pe/peh index for j+1
        jcol = (i_idx + k_idx).clamp(max=W - 1)            # (W,L) end-token index for h_j
        Z = (pe[:, end] - pe[:, :W].unsqueeze(-1)).clamp(min=1e-6)   # (B, W, L)
        N = peh[:, end] - peh[:, :W].unsqueeze(2)                    # (B, W, L, d)
        content = N / Z.unsqueeze(-1)                       # (B, W, L, d)
        hi = hw.unsqueeze(2).expand(B, W, L, d)
        hj = hw[:, jcol]                                    # (B, W, L, d)
        we = self.width(k_idx.expand(W, L)).unsqueeze(0).expand(B, W, L, -1).to(hw.dtype)
        rb = torch.bucketize(rec.float(), self.rec_bounds)  # (B, W) bin 0..4
        re = self.rec_emb(rb).to(hw.dtype)                  # (B, W, rec_dim)
        re_i = re.unsqueeze(2).expand(B, W, L, -1)
        re_j = re[:, jcol]
        sb = torch.bucketize(sem_rec.float(), self.sem_bounds)  # (B, W) bin 0..7
        se = self.sem_emb(sb).to(hw.dtype)                      # (B, W, sem_dim)
        se_i = se.unsqueeze(2).expand(B, W, L, -1)
        se_j = se[:, jcol]
        g = torch.cat([hi, hj, content, we, re_i, re_j, se_i, se_j], dim=-1)
        return self.ffnn(g).squeeze(-1)                     # (B, W, L) span logits
...
def collate(batch: list, tok, device: str, max_words: int = 120) -> tuple:
    ...
    L = MAX_MENTION_LEN
    tgt = torch.zeros(B, Wmax, L)                          # [b, start, offset k=j-i]
    for b in range(B):
    ...
    mask = ar_i + ar_k < lb                                # valid iff end token j=i+k is in-sentence
    rec = torch.zeros(B, Wmax, dtype=torch.long)
    sem_rec = torch.zeros(B, Wmax, dtype=torch.float)
    for b in range(B):
        r = batch[b]["rec"]
        sr = batch[b].get("sem_rec", [])
        for w in range(min(wlen[b], len(r))):
            rec[b, w] = r[w]
        for w in range(min(wlen[b], len(sr))):
            sem_rec[b, w] = sr[w]
    return (enc["input_ids"].to(device), enc["attention_mask"].to(device), word_first.to(device),
            tgt.to(device), mask.to(device), wlen, rec.to(device), sem_rec.to(device))
...
def evaluate(model, items, tok, device, bs=32, thr=0.5) -> dict:
    ...
    with torch.inference_mode():
        for s in range(0, len(items), bs):
            batch = items[s : s + bs]
            ids, attn, wf, _, _, wlen, rec, sem_rec = collate(batch, tok, device)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=device != "cpu"):
                logits = model(ids, attn, wf, rec, sem_rec)
    ...
    p = tp / (tp + fp + 1e-9)
    rc = tp / (tp + fn + 1e-9)
    return {"precision": p, "recall": rc, "f1": 2 * p * rc / (p + rc + 1e-9)}
...
def train(max_epochs: int = 15, bs: int = 32, patience: int = 4, enc_lr: float = 2e-5, head_lr: float = 1e-3,
    ...
    train_items = load_items("train", bge)
    val_items = load_items("validation", bge)
    test_items = load_items("test", bge)
    ...
    opt = optim.AdamW([
        ...
        {"params": list(model.attn.parameters()) + list(model.width.parameters()) + list(model.ffnn.parameters()), "lr": head_lr},
    ], weight_decay=0.01)
    ...
    for ep in range(max_epochs):
        ...
        for s in tqdm(range(0, len(train_items), bs), desc=f"ep{ep+1}"):
            ids, attn, wf, tgt, mask, _, rec, sem_rec = collate(train_items[s : s + bs], tok, device)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=device != "cpu"):
                logits = model(ids, attn, wf, rec, sem_rec)
        ...
        print(f"epoch {ep+1}/{max_epochs} loss {tot/nb:.4f} | val P {vm['precision']:.3f} R {vm['recall']:.3f} F1 {vm['f1']:.4f}", flush=True)
        if vm["f1"] > best + 1e-4:
            best, patience_ctr = vm["f1"], 0
            torch.save({"model": model.state_dict(), "val_f1": best}, STAGE0_FT_CKPT)
