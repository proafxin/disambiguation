...
from disambiguation.utilities.consts import CATEGORIES, STOPWORDS
from disambiguation.utilities.util import extract_clusters, extract_mentions_to_clusters, mask_tensor, get_pronoun_id
...
class LingMessModel(BertPreTrainedModel):
    ...
    def _prune_topk_mentions(self, mention_logits, attention_mask):
        ...
        k = (actual_seq_lengths * self.top_lambda).int()
        max_k = int(torch.max(k))

        _, topk_1d_indices = torch.topk(mention_logits.view(batch_size, -1), dim=-1, k=max_k)

        span_mask = self._get_span_mask(batch_size, k, max_k)
        seq_length_t = torch.tensor(seq_length, device=mention_logits.device)
        topk_1d_indices = (topk_1d_indices * span_mask) + (1 - span_mask) * (seq_length_t * seq_length_t - 1)
        sorted_topk_1d_indices, _ = torch.sort(topk_1d_indices, dim=-1)

        topk_mention_start_ids = sorted_topk_1d_indices.div(seq_length_t, rounding_mode='floor')
        topk_mention_end_ids = sorted_topk_1d_indices.remainder(seq_length_t)

        topk_mention_logits = mention_logits[torch.arange(batch_size).unsqueeze(-1).expand(batch_size, max_k),
                                             topk_mention_start_ids, topk_mention_end_ids]

        topk_mention_logits = topk_mention_logits.unsqueeze(-1) + topk_mention_logits.unsqueeze(-2)
    ...
    def _get_marginal_log_likelihood_loss(self, logits, labels, span_mask):
        gold_coref_logits = mask_tensor(logits, labels)                     

        gold_log_sum_exp = torch.logsumexp(gold_coref_logits, dim=-1)       
        all_log_sum_exp = torch.logsumexp(logits, dim=-1)                   
        losses = all_log_sum_exp - gold_log_sum_exp                         

        span_mask = span_mask.unsqueeze(1)                                  
        losses = losses * span_mask                                         

        per_span_loss = losses.mean(dim=-1)                                 

        loss_per_cat = per_span_loss.mean(dim=0)                            
    ...
    def _calc_mention_logits(self, start_mention_reps, end_mention_reps):
        start_mention_logits = self.mention_start_classifier(start_mention_reps).squeeze(-1)
        end_mention_logits = self.mention_end_classifier(end_mention_reps).squeeze(-1)      

        temp = self.mention_s2e_classifier(start_mention_reps)                              
        joint_mention_logits = torch.matmul(temp,
                                            end_mention_reps.permute([0, 2, 1]))            
        ...
        mention_logits = mask_tensor(mention_logits, mention_mask)                          
    ...
    def _get_all_labels(self, clusters_labels, categories_masks):
        ...
        all_labels = torch.cat((categories_labels, clusters_labels.unsqueeze(1)), dim=1)          

        zeros = torch.zeros((batch_size, self.num_cats + 1, max_k, 1), device=self.device)
        all_labels = torch.cat((all_labels, zeros), dim=-1)                                       
    ...
    def forward_transformer(self, batch):
        ...
        if 'leftovers' not in batch:
            ...
            attention_mask = attention_mask.view((docs, segments * segment_len))      
            sequence_output = sequence_output.view((docs, segments * segment_len, -1))
    ...
    def forward(self, batch, gold_clusters=None, return_all_outputs=False):
        ...
        categories_logits = torch.cat((categories_logits, torch.zeros((batch_size, self.num_cats, max_k, 1), device=self.device)), dim=-1)
