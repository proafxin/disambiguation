...
from disambiguation.utilities.util import extract_clusters, extract_mentions_to_clusters, mask_tensor
...
class FCorefModel(BertPreTrainedModel):
    ...
    def _get_marginal_log_likelihood_loss(self, coref_logits, cluster_labels_after_pruning, span_mask):
        ...
        all_log_sum_exp = torch.logsumexp(coref_logits, dim=-1)
    ...
    def _calc_coref_logits(self, top_k_start_coref_reps, top_k_end_coref_reps):
        temp = self.antecedent_s2s_classifier(top_k_start_coref_reps)
        top_k_s2s_coref_logits = torch.matmul(temp,
                                              top_k_start_coref_reps.permute([0, 2, 1]))

        temp = self.antecedent_e2e_classifier(top_k_end_coref_reps)
        top_k_e2e_coref_logits = torch.matmul(temp,
                                              top_k_end_coref_reps.permute([0, 2, 1]))

        temp = self.antecedent_s2e_classifier(top_k_start_coref_reps)
        top_k_s2e_coref_logits = torch.matmul(temp,
                                              top_k_end_coref_reps.permute([0, 2, 1]))

        temp = self.antecedent_e2s_classifier(top_k_end_coref_reps)
