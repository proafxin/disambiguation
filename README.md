...
The `disambiguation` Python package provides an easy and fast API for coreference information with only few lines of code without any prepossessing steps.
...
```python
from disambiguation import FCoref
...
```python
from disambiguation import LingMessCoref
...
```python
from disambiguation import spacy_component
import spacy
...
doc = nlp(      # for multiple texts use nlp.pipe
   text,
   component_cfg={"disambiguation": {'resolve_text': True}}
)
...
```python
from disambiguation import TrainingArgs, CorefTrainer
